...
//                console.log('row-' + action, rowData, index);
                this.$emit('row-' + action, _.cloneDeep(rowData), index)
            },
            customRawValue: function (rowData, columnSpec, key) {
                var val = _.get(rowData, key, '?');

                if (columnSpec.customValue && _.isFunction(columnSpec.customValue)) {
                    val = columnSpec.customValue(val, rowData);
                }

                if (columnSpec.customFilter && _.isFunction(columnSpec.customFilter)) {
            ...
            actionIsShown: function (action, rowData) {
                if (action.hideOn && _.isFunction(action.hideOn)) {
                    return !action.hideOn(rowData)
                }
                else if (action.showOn && _.isFunction(action.showOn)) {
